...
export default async function Dashboard({
  ...
  searchParams: Promise<{ role?: string; user?: string; sid?: string }>;
}) {
  ...
  if (!params.sid) {
    ...
    url.set('sid', newSid);
    redirect(`/dashboard?${url.toString()}`);
  }
  const role = params.role || 'individual';
  const username = params.user || 'anonymous';
  const sessionId = params.sid;
  
  const grievances = getGrievances().filter(g => {
    // Admin sees everything, others see only their session's records
    if (role === 'admin') return true;
    return g.Session_ID === sessionId && g.Raised_By === username;
  });

  const limit = role === 'head' ? 5 : (role === 'admin' ? Infinity : 1);
  const userCount = grievances.length; 
        ...
        {role !== 'admin' && (
          <div className="lg:col-span-1">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sticky top-24 shadow-2xl shadow-black/50">
              ...
              {canSubmit ? (
                <GrievanceForm username={username} sessionId={sessionId} />
              ) : (
